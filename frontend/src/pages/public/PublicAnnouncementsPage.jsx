  ...
  return <>
    ...
    <section className="public-section bg-slate-50"><div className="public-container">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">{paginatedAnnouncements.map((item) => <article key={item._id} role="button" tabIndex={0} onClick={() => setSelectedAnnouncement(item)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); setSelectedAnnouncement(item); } }} className="flex min-h-[360px] min-w-0 cursor-pointer flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm transition hover:-translate-y-0.5 hover:shadow-md focus:outline-none focus:ring-2 focus:ring-emerald-500">
        {item.media?.[0] ? <div className="relative h-60 shrink-0 overflow-hidden border-b border-slate-100 bg-slate-900"><img src={item.media[0].dataUrl} alt="" aria-hidden="true" loading="lazy" className="absolute inset-0 h-full w-full scale-110 object-cover opacity-40 blur-md" /><div className="relative z-10 grid h-full w-full place-items-center bg-slate-950/10"><img src={item.media[0].dataUrl} alt={item.media[0].name || item.title} loading="lazy" className="h-full w-full object-contain" style={{ objectFit: "contain", objectPosition: "center" }} /></div></div> : <div className="grid h-60 shrink-0 place-items-center bg-emerald-50 text-emerald-200"><Megaphone size={40} /></div>}
        <div className="flex flex-1 flex-col p-3"><time className="flex items-center gap-1.5 text-[10px] font-semibold text-slate-500"><CalendarClock size={12} />{new Date(item.createdAt).toLocaleDateString()}</time><h2 className="mt-2 line-clamp-2 text-sm font-black leading-tight text-slate-950">{item.title}</h2><p className="mt-2 line-clamp-2 text-[11px] leading-4 text-slate-600">{item.message}</p><span className="mt-3 flex items-center gap-1 border-t border-slate-100 pt-2.5 text-[10px] font-bold text-emerald-700">Read announcement <ArrowRight size={11} /></span></div>
      </article>)}</div>
