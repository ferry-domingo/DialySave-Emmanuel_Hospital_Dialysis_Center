...
import { Activity, ArrowRight, BadgeCheck, CheckCircle2, Clock3, Compass, Download, Eye, HeartHandshake, Mail, MapPin, MonitorCheck, Phone, Search, Share2, ShieldCheck, Sparkles, Stethoscope } from "lucide-react";
...
const TechnologySlider = ({ category = "technology", fallback = true, label = "Facilities gallery" }) => {
  ...
  return <div className="technology-gallery-showcase">
    <div className="technology-gallery-heading"><div><p className="public-eyebrow dark">Complete facility gallery</p><h3>Explore every uploaded space and system.</h3></div><span>{photos.length} {photos.length === 1 ? "photo" : "photos"}</span></div>
    <div className="technology-thumbnail-grid" aria-label={label}>
      {photos.map((photo, index) => <button key={photo._id} type="button" onClick={() => setPreview({ src: photo.imageUrl, alt: photo.caption || photo.name || `Facility photo ${index + 1}` })} aria-label={`View ${photo.caption || photo.name || `facility photo ${index + 1}`}`}><img src={photo.imageUrl} alt={photo.caption || photo.name || `Facility photo ${index + 1}`} loading="lazy"/><span><b>{String(index + 1).padStart(2, "0")}</b>{photo.caption || photo.name || "Facility image"}</span></button>)}
    </div>
...
export const ContactPage = () => {
  ...
  return <><PageHero eyebrow="Contact & visits" title="We are ready to help you plan your next step." text="Reach the center for treatment inquiries, scheduling guidance, or help accessing the secure portal." position="right"/><section className="public-section"><div className="public-container contact-grid"><div><p className="public-eyebrow dark">Get in touch</p><h2>Talk with our care team.</h2><p>Use the confirmed contact details provided by the center for appointments and patient inquiries.</p><div className="contact-cards">{phoneHref ? <a href={phoneHref}><Phone/><div><strong>Call the center</strong><span>{contact.phone}</span></div></a> : <article><Phone/><div><strong>Call the center</strong><span>Contact number will be available soon</span></div></article>}{contact.email ? <a href={`mailto:${contact.email}`}><Mail/><div><strong>Email support</strong><span>{contact.email}</span></div></a> : <article><Mail/><div><strong>Email support</strong><span>Email address will be available soon</span></div></article>}<a href={MAP_URL} target="_blank" rel="noreferrer" aria-label="Open Emmanuel Hospital Dialysis Center in Google Maps"><MapPin/><div><strong>Visit us</strong><span>Emmanuel Hospital Dialysis Center</span></div></a><article><Clock3/><div><strong>Center hours</strong><span>Contact the center for the current schedule</span></div></article></div></div><aside><h3>Already registered?</h3><p>Access records, messages, alerts, and role-specific tools through the secure portal.</p><Link to="/login" className="public-primary">Sign in to portal <ArrowRight size={17}/></Link><small>Do not share your password or account ID with anyone.</small></aside></div></section></>;
};
