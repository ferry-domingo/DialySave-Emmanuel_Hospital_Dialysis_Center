  ...
  return <div className="technology-gallery-showcase">
    ...
    <div className="technology-thumbnail-grid" aria-label={label}>
      {photos.map((photo, index) => {
        const thumbnailLabel = photo.caption || photo.name || "";
        const imageAlt = photo.caption || photo.name || `Facility photo ${index + 1}`;
        return <button key={photo._id} type="button" onClick={() => setPreview({ src: photo.imageUrl, alt: imageAlt })} aria-label={`View ${imageAlt}`}><img src={photo.imageUrl} alt={imageAlt} loading="lazy" /><span><b>{String(index + 1).padStart(2, "0")}</b>{thumbnailLabel}</span></button>;
      })}
    </div>
